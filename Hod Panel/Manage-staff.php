...
{
$adminid=$_GET['id'];
$msg=mysqli_query($conn,"delete from staff_data where id='$adminid'");
if($msg)
{
...
<?php 
      if (isset($_POST["Submit"])) {
        ...
        if($name != "" || $email != "" || $AYear != "" || $Gender != "" ){
          $query = "SELECT * FROM staff_data WHERE (full_name = '$name' AND Branch = '$CurrentBranch') OR (AYear = '$AYear' AND Branch = '$CurrentBranch') || (Gender = '$Gender' AND Branch = '$CurrentBranch') OR (email = '$email' AND Branch = '$CurrentBranch')";
        ...
        $ret=mysqli_query($conn,"SELECT * FROM staff_data WHERE Branch = '$CurrentBranch'");
                $cnt=1;
  ...
  <!--
  <tbody>
    <?php $ret=mysqli_query($conn,"select * from staff_data");
                $cnt=1;
                while($row=mysqli_fetch_array($ret))
                {?>
    <tr>
