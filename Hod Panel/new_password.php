...
if(isset($_POST['submit']))
{
  if(isset($_POST['newpass'])) {
    ...
    $sql=mysqli_query($conn,"SELECT password FROM hod_data where email='$current_Email'");
    $num=mysqli_fetch_array($sql);
    if($num>0) {
      $newpass=md5($_POST['newpass']);
      $newpass1=md5($_POST['newpass1']);
      if ($newpass === $newpass1) {
        $ret=mysqli_query($conn,"UPDATE hod_data SET password='$newpass' WHERE email='$current_Email'");
