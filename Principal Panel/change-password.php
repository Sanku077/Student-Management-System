...
if(isset($_POST['Submit']))
{
  if(isset($_POST['oldpass']) || isset($_POST['newpass']) || isset($_POST['confirmpassword'])) {
    ...
    $sql=mysqli_query($conn,"SELECT password FROM principal_data where password='$oldpassword'");
  $num=mysqli_fetch_array($sql);
  if($num>0) {
    $adminid=$_SESSION['id'];
    $newpass=md5($_POST['newpass']);
    $ret=mysqli_query($conn,"update principal_data set password='$newpass' where id='$adminid'");
