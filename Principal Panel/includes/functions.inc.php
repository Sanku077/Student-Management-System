...
function emptyInputPost($fullname,$heading,$content,$files) {

$results;

if (empty($fullname) || empty($heading) || empty($content)) {
        $results==true;
    }
    else{
        $results=false;
...
function createPost($conn,$fullname,$heading,$content,$files) {

    $sql = "INSERT INTO announcement (full_name,heading,content,files) VALUES (?,?,?,?)";
    $stmt = mysqli_stmt_init($conn);
    if (!mysqli_stmt_prepare($stmt,$sql)) {
    ...
    mysqli_stmt_bind_param($stmt,"ssss",$fullname,$heading,$content,$files);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
